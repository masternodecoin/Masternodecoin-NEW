    ...
    if (wtx.IsCoinStake()) {
        ...
        for (const CTxOut txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecordEx sub(hash, nTime);
                sub.type = TransactionRecordEx::ZerocoinSpend_Change_zMTNC;
...
std::string formatTxType(const TransactionRecordEx::Type type)
{
    switch (type) {
		...
		case TransactionRecordEx::ZerocoinMint:
			return std::string("Converted MTNC to zMTNC");
		case TransactionRecordEx::ZerocoinSpend:
			return std::string("Spent zMTNC");
		case TransactionRecordEx::RecvFromZerocoinSpend:
			return std::string("Received MTNC from zMTNC");
		case TransactionRecordEx::ZerocoinSpend_Change_zMTNC:
			return std::string("Minted Change as zMTNC from zMTNC Spend");
		case TransactionRecordEx::ZerocoinSpend_FromMe:
			return std::string("Converted zMTNC to MTNC");
