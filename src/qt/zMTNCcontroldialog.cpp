...
#include "ui_zMTNCcontroldialog.h"
...
std::list<std::string> ZMTNCControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMTNCControlDialog::listMints;

ZMTNCControlDialog::ZMTNCControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMTNCControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMTNCControlDialog::~ZMTNCControlDialog()
{
    delete ui;
}

void ZMTNCControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMTNCControlDialog::updateList()
{
...
void ZMTNCControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMTNCControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMTNC_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMTNCControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMTNCControlDialog::GetSelectedMints()
{
...
void ZMTNCControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
