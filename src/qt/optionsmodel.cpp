...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::MTNC);
    ...
    if (!settings.contains("nAnonymizeMTNCAmount"))
        settings.setValue("nAnonymizeMTNCAmount", 1000);

    nAnonymizeMTNCAmount = settings.value("nAnonymizeMTNCAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeMTNCAmount"))
        SoftSetArg("-anonymizeMTNCamount", settings.value("nAnonymizeMTNCAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in MTNC.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMTNCAmount:
            return QVariant(nAnonymizeMTNCAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeMTNCAmount:
            nAnonymizeMTNCAmount = value.toInt();
            settings.setValue("nAnonymizeMTNCAmount", nAnonymizeMTNCAmount);
            emit anonymizeMTNCAmountChanged(nAnonymizeMTNCAmount);
            break;
        case CoinControlFeatures:
