...
#if defined(NDEBUG)
#error "MTNC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uMTNC) are considered zero fee (for relaying and mining)
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zMTNC with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMTNC spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zMTNC spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight == 0) {
            ...
            nSubsidy = 117000000 * COIN;
        } else if (nHeight < 1000 && nHeight > 10) {
            nSubsidy = 250 * COIN;
        } else if (nHeight < 103680 && nHeight >= 1000) {
            nSubsidy = 700 * COIN;
        } else if (nHeight < 207360 && nHeight >= 103680) {
            nSubsidy = 600 * COIN;
        } else if (nHeight < 311040 && nHeight >= 207360) {
            nSubsidy = 500 * COIN;
        } else if (nHeight < 414720 && nHeight >= 311040) {
            nSubsidy = 400 * COIN;
        } else if (nHeight < 518400 && nHeight >= 414720) {
            nSubsidy = 350 * COIN;
        } else if (nHeight < 622080 && nHeight >= 518400) {
            nSubsidy = 300 * COIN;
        } else if (nHeight < 725760 && nHeight >= 622080) {
            nSubsidy = 250 * COIN;
        } else if (nHeight < 829440 && nHeight >= 725760) {
            nSubsidy = 200 * COIN;
        } else {
            nSubsidy = 150 * COIN;
        }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from MTNC
...
{
    RenameThread("MTNC-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMTNCMinted()
{
...
void RecalculateZMTNCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMTNC supply
...
bool RecalculateMTNCSupply(int nHeightStart)
{
...
bool UpdateZMTNCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateMTNCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zMTNC money supply in the block index
    if (!UpdateZMTNCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zMTNC supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMTNCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MTNC to zMTNC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMTNC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMTNC serial %s in block\n Block: %s",
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (isBlockFromFork) {
            ...
            // Inputs
            std::vector<CTxIn> MTNCInputs;

            for (CTxIn stakeIn : stakeTxIn.vin) {
                MTNCInputs.push_back(stakeIn);
            }
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMTNC mints and %d zMTNC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // MTNC: We use certain sporks during IBD, so check to see if they are
